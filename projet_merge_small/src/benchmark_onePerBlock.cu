...
void benchmark_mergeOnePerBlock(int d, int N) {
    int sizeA = d / 2;
    int totalA = N * sizeA;
    int totalM = N * d;
    ...
    int threadsPerBlock = d; // 1 fusion par bloc
    int numBlocks = N;
    ...
    mergeSmallOnePerBlock_k<<<numBlocks, threadsPerBlock>>>(d_A, d_B, d_M, d, N);
    ...
    printf("[OnePerBlock] d = %4d | N = %6d | Time = %7.3f ms\n", d, N, milliseconds);
...
int main() {
    ...
    for (int i = 0; i < 4; ++i) {
        benchmark_mergeOnePerBlock(d_values[i], N);
    }
